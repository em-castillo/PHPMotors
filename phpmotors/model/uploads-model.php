...
// Obtains all thumbnail image path information from the images table based on vehicleId (invId)
function getThumbnailImg($invId)
{
    $db = phpmotorsConnect();
    ...
    $stmt->bindValue(':invId', $invId, PDO::PARAM_INT);
    $stmt->execute();
    $thumbImg = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $stmt->closeCursor();
    return $thumbImg;
}
