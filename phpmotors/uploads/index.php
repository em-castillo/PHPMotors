...
switch ($action) {
    ...
    case 'upload':
        ...
        if ($imageCheck) {
            $message = '<p id="text">An image by that name already exists.</p>';
        } elseif (empty($invId) || empty($imgName)) {
            $message = '<p id="text">You must select a vehicle and image file for the vehicle.</p>';
        } else {
            ...
            if ($result) {
                $message = '<p id="text">The upload succeeded.</p>';
            } else {
                $message = '<p id="text">Sorry, the upload failed.</p>';
            }
        }
    ...
    case 'delete':
        ...
        if ($remove) {
            $message = "<p id='text'>$filename was successfully deleted.</p>";
        } else {
            $message = "<p id='text'>$filename was NOT deleted.</p>";
        }
    ...
    default:
        ...
        if (count($imageArray)) {
            ...
            $imageDisplay = '<p id="text">Sorry, no images could be found.</p>';
        }
